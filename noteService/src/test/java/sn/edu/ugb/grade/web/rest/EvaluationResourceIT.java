...
class EvaluationResourceIT {
    ...
    public static Evaluation createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, SessionExamen.class).isEmpty()) {
            sessionExamen = SessionExamenResourceIT.createEntity();
            em.persist(sessionExamen);
            em.flush();
        } else {
            sessionExamen = TestUtil.findAll(em, SessionExamen.class).get(0);
        }
        evaluation.setSession(sessionExamen);
        return evaluation;
    }
    ...
    public static Evaluation createUpdatedEntity(EntityManager em) {
        ...
        SessionExamen sessionExamen;
        if (TestUtil.findAll(em, SessionExamen.class).isEmpty()) {
            sessionExamen = SessionExamenResourceIT.createUpdatedEntity();
            em.persist(sessionExamen);
            em.flush();
        } else {
            sessionExamen = TestUtil.findAll(em, SessionExamen.class).get(0);
        }
        updatedEvaluation.setSession(sessionExamen);
