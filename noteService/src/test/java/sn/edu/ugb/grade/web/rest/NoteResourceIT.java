...
class NoteResourceIT {
    ...
    public static Note createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, Evaluation.class).isEmpty()) {
            evaluation = EvaluationResourceIT.createEntity(em);
    ...
    public static Note createUpdatedEntity(EntityManager em) {
        ...
        Evaluation evaluation;
        if (TestUtil.findAll(em, Evaluation.class).isEmpty()) {
            evaluation = EvaluationResourceIT.createUpdatedEntity(em);
            em.persist(evaluation);
            em.flush();
        } else {
            evaluation = TestUtil.findAll(em, Evaluation.class).get(0);
        }
        updatedNote.setEvaluation(evaluation);
        return updatedNote;
    }
    ...
    void partialUpdateNoteWithPatch() throws Exception {
        ...
        partialUpdatedNote.dateSaisie(UPDATED_DATE_SAISIE);
