...
class CurriculumResourceIT {
    ...
    public static Curriculum createEntity(EntityManager em) {
        Curriculum curriculum = new Curriculum()
            ...
            .uniteId(DEFAULT_UNITE_ID)
            .semestreId(DEFAULT_SEMESTRE_ID);
        // Add required entity
        Semestre semestre;
        if (TestUtil.findAll(em, Semestre.class).isEmpty()) {
            semestre = SemestreResourceIT.createEntity();
            em.persist(semestre);
            em.flush();
        } else {
            semestre = TestUtil.findAll(em, Semestre.class).get(0);
        }
        curriculum.setSemestre(semestre);
        return curriculum;
    }
    ...
    public static Curriculum createUpdatedEntity(EntityManager em) {
        Curriculum updatedCurriculum = new Curriculum()
            ...
            .uniteId(UPDATED_UNITE_ID)
            .semestreId(UPDATED_SEMESTRE_ID);
        // Add required entity
        Semestre semestre;
        if (TestUtil.findAll(em, Semestre.class).isEmpty()) {
            semestre = SemestreResourceIT.createUpdatedEntity();
            em.persist(semestre);
            em.flush();
        } else {
            semestre = TestUtil.findAll(em, Semestre.class).get(0);
        }
        updatedCurriculum.setSemestre(semestre);
    ...
    void checkAnneeAcademiqueIsRequired() throws Exception {
        ...
        // set the field null
        curriculum.setAnneeAcademique(null);
    ...
    void partialUpdateCurriculumWithPatch() throws Exception {
        ...
        partialUpdatedCurriculum.anneeAcademique(UPDATED_ANNEE_ACADEMIQUE).filiereId(UPDATED_FILIERE_ID).uniteId(UPDATED_UNITE_ID);
