...
class MatiereResourceIT {
    ...
    private static final Long DEFAULT_UNITE_ID = 1L;
    private static final Long UPDATED_UNITE_ID = 2L;
    ...
    public static Matiere createEntity(EntityManager em) {
        Matiere matiere = new Matiere().nom(DEFAULT_NOM).heures(DEFAULT_HEURES).credits(DEFAULT_CREDITS).uniteId(DEFAULT_UNITE_ID);
        // Add required entity
        UniteEnseignement uniteEnseignement;
        if (TestUtil.findAll(em, UniteEnseignement.class).isEmpty()) {
            uniteEnseignement = UniteEnseignementResourceIT.createEntity(em);
            em.persist(uniteEnseignement);
            em.flush();
        } else {
            uniteEnseignement = TestUtil.findAll(em, UniteEnseignement.class).get(0);
        }
        matiere.setUnite(uniteEnseignement);
        return matiere;
    }
    ...
    public static Matiere createUpdatedEntity(EntityManager em) {
        Matiere updatedMatiere = new Matiere().nom(UPDATED_NOM).heures(UPDATED_HEURES).credits(UPDATED_CREDITS).uniteId(UPDATED_UNITE_ID);
        // Add required entity
        UniteEnseignement uniteEnseignement;
        if (TestUtil.findAll(em, UniteEnseignement.class).isEmpty()) {
            uniteEnseignement = UniteEnseignementResourceIT.createUpdatedEntity(em);
            em.persist(uniteEnseignement);
            em.flush();
        } else {
            uniteEnseignement = TestUtil.findAll(em, UniteEnseignement.class).get(0);
        }
        updatedMatiere.setUnite(uniteEnseignement);
    ...
    @Test
    @Transactional
    void checkUniteIdIsRequired() throws Exception {
        long databaseSizeBeforeTest = getRepositoryCount();
        // set the field null
        matiere.setUniteId(null);
    ...
    void getAllMatieres() throws Exception {
        ...
        restMatiereMockMvc
            ...
            .andExpect(jsonPath("$.[*].uniteId").value(hasItem(DEFAULT_UNITE_ID.intValue())));
    }
    ...
    void getMatiere() throws Exception {
        ...
        restMatiereMockMvc
            ...
            .andExpect(jsonPath("$.uniteId").value(DEFAULT_UNITE_ID.intValue()));
    }
    ...
    void putExistingMatiere() throws Exception {
        ...
        updatedMatiere.nom(UPDATED_NOM).heures(UPDATED_HEURES).credits(UPDATED_CREDITS).uniteId(UPDATED_UNITE_ID);
    ...
    void partialUpdateMatiereWithPatch() throws Exception {
        ...
        partialUpdatedMatiere.nom(UPDATED_NOM).heures(UPDATED_HEURES).credits(UPDATED_CREDITS);
    ...
    void fullUpdateMatiereWithPatch() throws Exception {
        ...
        partialUpdatedMatiere.nom(UPDATED_NOM).heures(UPDATED_HEURES).credits(UPDATED_CREDITS).uniteId(UPDATED_UNITE_ID);
